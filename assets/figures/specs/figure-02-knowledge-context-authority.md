...
5. Deterministic verification box and agentic review box, both fed from the artefact.
6. Evidence box, fed by both checks, stating what evidence is and what it is not.
7. Critical Expert validation box, fed directly from the artefact, carrying the authority mark and naming its three objects of assessment.
...
- Deterministic verification, formalised checks
- Agentic review, model-mediated checking
- Evidence, reports about the artefact, never an authorisation
- Critical Expert validation, source fidelity, design, interpretation
...
Two structural relations carry the caption's central distinction. Deterministic verification and agentic review reach the Evidence box by solid arrows at primary weight, because they produce evidence. Evidence reaches Critical Expert validation by a dashed arrow at secondary weight labelled "informs", because it enters a judgement without settling it. Critical Expert validation reaches acceptance by a solid arrow at primary weight labelled "authorises".

## Visual grammar

Solid line with a filled head marks operational flow and authorising acts. Dashed line with an open head marks a supporting relation, which here covers the evidence relation and the write-back. The filled disc inside a circle appears once, on Critical Expert validation. The light grey fill appears once, on the Acceptance box, because acceptance is the state the caption ends on. Nesting marks a container whose named constituents are drawn inside it, which applies to the knowledge base and to the harness.
...
- Evidence is not acceptance. The dashed arrow into validation is the only path evidence takes, and it does not reach the acceptance box.
...
- Exactly one filled disc, on Critical Expert validation.
