...
4. When rendering settles, `app.js` dispatches `promptotyping:sections-ready` (the two modules boot on it), then renders the paper, and finally runs `setupScrollSpy` and `handleHash`.
...
**Method pages.** `anwendung`, `workflow`, `artefakt` and `verifikation` are static content sections rendered by `renderMarkdownInto` from `_content/{slug}.md`, resolvable as subpaths of the same name. The first three are written against the canonical paper; `workflow` walks the video-documented session through the four phases and is sourced from the cleaned scripts in the vault's representation layer. Since 2026-07-25 the reading column puts the method first and the paper last, so the site answers what Promptotyping is and how it is applied before it argues why (`knowledge/plan-site.md`).
