...
**Method pages.** `anwendung`, `artefakt` and `verifikation` are static content sections rendered by `renderMarkdownInto` from `_content/{slug}.md`, written against the canonical paper and resolvable as subpaths of the same name. Since 2026-07-25 the reading column puts the method first and the paper last, so the site answers what Promptotyping is and how it is applied before it argues why (`knowledge/plan-site.md`).
