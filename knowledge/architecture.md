...
- **Scripts**: `assets/js/app.js` (an IIFE that exposes shared helpers on `window.PromptotypingApp`) plus three plain-script modules under `assets/js/modules/`. No ES-module `import`. The shell declares the first two; `app.js` injects `term-index.js` at runtime and awaits it, because `index.html` and `404.html` must stay free of markup that could drift apart.
...
3. `setupSidePanel()` and `setupGlossarInteraction()` wire the reusable slide-in panel and the glossar hover/click delegation. The sidebar has no toggle; it is always visible.
...
5. A `Promise.all` renders the static pages in parallel: `renderGlossar` (must finish before paper sections so their glossar triggers can be marked), `renderMarkdownInto("ueberblick", …, buildSpecIndex)`, the four method pages, `renderVorlagen`, the `konvention` render, `renderPraxis`, `renderSkills`, and the `arbeitsumgebung` render. `renderUseCasesHost` injects the host markup for the case-study module.
6. When rendering settles, `loadLateModules` injects and awaits the module scripts the shell does not declare, then `app.js` dispatches `promptotyping:sections-ready` (the three modules boot on it), renders the paper, then the vault, then runs `handleHash` against the fully rendered DOM, and finally dispatches `promptotyping:content-ready`, the event at which every addressable piece of content stands in the DOM. The term index listens for the second event, because it scans the rendered pages rather than a data file alone.
