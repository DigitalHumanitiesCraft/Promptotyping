...
6. When rendering settles, `app.js` dispatches `promptotyping:sections-ready` (the two modules boot on it), renders the paper, then the vault, and finally runs `handleHash` against the fully rendered DOM.
