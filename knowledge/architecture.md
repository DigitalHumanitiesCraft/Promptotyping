...
3. `setupSidePanel()`, `setupNavToggle()`, and `setupGlossarInteraction()` wire the reusable slide-in panel, the mobile sidebar drawer, and the glossar hover/click delegation.
...
6. When rendering settles, `app.js` dispatches `promptotyping:sections-ready` (the two modules boot on it), renders the paper, then the vault, and finally rebuilds the rail and runs `handleHash`.
