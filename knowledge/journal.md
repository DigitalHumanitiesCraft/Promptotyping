...
Diese Runde begann als Bildarbeit und endete als Textarbeit. Der Ertrag im Paper ist grösser als der auf dem Papier, weil eine Abbildung eine Unterscheidung erzwingt, die Prosa offen lassen kann.

**Fünf unscharfe Stellen sind beim Zeichnen aufgefallen.** Die Unterscheidung zwischen kuratierten und generierten Dokumenten stand als Eigenschaft des deklarativen Typs; sie läuft unabhängig von allen drei Typen und ist eine Eigenschaft der Provenienz, entschieden durch die Frage, ob der nächste Lauf eines benannten Kommandos die Datei überschreibt. Die Beschreibung einer Projektpipeline gehört zu den deklarativen Dokumenten und nicht zu den Prozessdokumenten. Der Adressat wechselt mit dem Dokumenttyp, während die doppelte Lesbarkeit über den ganzen Satz konstant bleibt. In 6.2 lagen Rangordnung und Prüfebene ineinander. Und der Rang der maschinellen Prüfinstanz war gesetzt statt begründet.

**Aus dem adversarial machine review wurde ein LLM Review.** Der Operator hat die martialische Formulierung beanstandet und den Anschluss an LLM-as-a-Judge vorgeschlagen. Beides ist eingelöst, mit Zheng et al. 2023 im Verzeichnis, und die dort dokumentierten Verzerrungen tragen jetzt die Begründung dafür, warum die Instanz unter der menschlichen Verifikation rangiert. Mein Zwischenvorschlag `machine review` war unbrauchbar, weil die unterste Prüfebene ebenfalls eine Maschine ist. Im Record sind es ausschliesslich LLMs, also heisst es so.

**Die dritte Abbildung ist eine Tabelle geworden.** Drei nach Rang geordnete Posten sind kein Diagramm; ein Leser hält sie in Prosa. Die Tabelle in 6.2 führt fünf Zeilen über vier Spalten, Ebene, Begriff, Entscheidungsinstanz und Autonomiezone, und sie deckte beim Bau auf, dass die Abbildung die Erfüllung der Anforderungen als Prüfebene ganz unterschlagen hatte. Die Projektübersicht in 5.2 trägt seither eine Beschriftung als Table 1.

**Exploration ist nicht optional.** Der Operator hat das Argument geliefert: entweder der Fachwissenschaftler exploriert selbst, oder wer für ihn baut, muss es tun. Was variiert, ist die Tiefe, und wo die Daten vertraut sind, ist die Arbeit kurz, weil das Verständnis schon vorliegt. Eine Phase als optional zu deklarieren lädt dort zum Auslassen ein, wo sie am nötigsten ist.

**Das Bildmodell hat eine Belastungsgrenze, und Textzerfall ist ihr Signal.** Die Phasenabbildung kippte zweimal, als die untere Zone einen Knoten mit vier gerichteten Verbindungen bekam. Die Rücklaufbögen hängten sich an das durchlaufende Band, so dass Korrekturen aus dem Journal in die Phasen zu fliessen schienen, und im nächsten Durchlauf lief der Rücklauf invertiert in Vorwärtsrichtung. Beide Male zerfielen gleichzeitig sieben Beschriftungen. Die Lehre ist, pro Durchlauf eine Sache zu ändern, und die brauchbare Fassung war die einfachste.

**Prompts sind nicht komprimierbar, Bedeutungserklärungen schon.** Etwa die Hälfte meiner Prompts war Ballast, weil ich dem Modell erklärt habe, was eine Form bedeutet. Ein Bildmodell schliesst daraus nichts. Was seinen Platz verdient, ist der wörtliche Text, die Geometrie und die Verbote, und jedes Verbot steht für einen konkreten Fehldurchlauf.

**Farbe bleibt draussen.** Nach der eigenen Regel bedeutet Farbe genau eine Sache, die epistemische Funktion eines Artefakts, und sie erscheint auf der Artefakt-Seite und an der Kartenkante, sonst nirgends. Keine der Abbildungen handelt davon. Meine frühere Zusage, die fünf Interface-Kategorien für 4.2 farbig zu tabellieren, ziehe ich damit zurück; sie wäre der dritte Farbort gewesen.

**Ablage.** Die Bilder liegen unter `assets/figures/` und werden aus `paper.md` repo-relativ referenziert, was trägt, weil `404.html` jede Subpath-Adresse auf den Site-Root zurückwirft. In der GitHub-Vorschau bleiben sie deshalb leer, der bewusste Preis für Portabilität ohne feste Domain. Der Dunkelmodus dreht die weisse Platte per CSS-Filter um, statt eine zweite Datei zu führen. Die Erzeugungsvorschriften stehen in `assets/figures/PROVENANCE.md`, nicht im Vault; ein Prompt behauptet nichts über die Welt und lässt sich in nichts verankern, also wäre die Quellen-Distillat-Claim-Kette ein Kategorienfehler gewesen.

**Kleinkram nachgezogen.** Vierzehn Befüll-Beispiele in den Vorlagen führten `generated-with: [Werkzeug, falls relevant]` und zeigen jetzt das dokumentierte Klammerformat. Zwei Zeitformeln im Paper sind an einen festen Punkt gebunden statt an eine mitlaufende Dauer. Der Herkunftssatz zum Übersetzungsproblem steht in Abschnitt 1 und weist es dem Requirements Engineering zu.

**Offen.** Der Herkunftssatz zum Requirements Engineering trägt keine Fundstelle und braucht eine. In Abbildung 1 fehlen die drei Wörter über den Rücklaufaufstiegen und die offene Ringmarke für den Agenten, die der gefüllten Scheibe ihren Gegenpart gäbe; ohne sie ruht deren Bedeutung allein auf der Bildunterschrift. Abbildung 2 hat ihre Diagnosezeile auf Operator-Entscheidung verloren, was die Begründung schwächt, mit der 3.3 die Typologie einführt.

## 2026-07-26, siebte Runde — Lichtmodus, Glossar-Tooltip und Referenzanker

Diese Runde ist nachgetragen. Sie liegt in den Commits `77b4ec8` und `e3dedaa`, und keiner der beiden hat ein Wissensdokument angefasst. Der Befund darüber steht in der achten Runde.

**Der Lichtmodus ist die erste Ansicht.** Das Theme-Prelude schreibt das `data-theme`-Attribut jetzt immer, den gespeicherten Wert wo einer vorliegt und sonst `light`. Damit entscheidet die Systempräferenz nicht mehr, was ein Erstbesucher sieht, während der `prefers-color-scheme`-Block im Stylesheet stehen bleibt und vom gesetzten Attribut überstimmt wird.

**Das Glossar-Panel ist durch einen Tooltip ersetzt.** Ein Begriff im Lesetext öffnet eine Sprechblase mit der Kurzdefinition und einem Link auf den vollen Eintrag. Der Weg über das Seitenpanel ist aus diesem Fluss heraus. Die Sprechblase bleibt offen, solange der Zeiger in ihr steht, weil der Link sonst unerreichbar wäre, und auf Berührungsgeräten trägt der Klick die ganze Interaktion.

**Jede Referenz hat eine eigene Adresse.** Die Einträge des Literaturverzeichnisses tragen eine ID aus Nachname und Jahr, und eine Zitation im Text landet auf dem Eintrag statt am Kopf der Liste. Zwei Fehler sind dabei aufgefallen. Ein verstümmeltes Wortgrenzen-Escape im Zitationsmuster hatte 23 von 78 Einträgen ohne Anker gelassen, und Monatsklammern wie „(January 2026)" waren von Anfang an als Zitationen verlinkt worden.

**Die Abbildungskomponente nimmt das Bild, das das Markdown schon trägt.** Bild und folgender Beschriftungsabsatz werden zu einem `figure` mit `figcaption` zusammengezogen, erkannt am führenden fetten `Figure {n}.`. Eine Beschriftung ohne Bild davor bleibt ein Absatz, und genau so verfällt eine nicht gezeichnete Abbildung.

## 2026-07-26, achte Runde — Die offenen Entscheidungen aufgelöst, die Vorlage Technology aufgenommen

Ziel war, `entscheidungen-offen.md` abzuarbeiten und zu löschen, und aus dem Repository die Widersprüche zu nehmen. Der Ertrag liegt weniger im Abarbeiten als in der Prüfung, weil das Dokument selbst zur Driftquelle geworden war.

**Drei von zehn Einträgen waren sachlich falsch, und alle drei auf dieselbe Weise.** Sie haben eine vorhandene Quelle nicht konsultiert und den Stand aus dem Gedächtnis rekonstruiert. C1 und C2 hatte das Dokument bereits selbst korrigiert. Bei C3 war die Prüfung fällig, und sie fiel gegen den Eintrag aus. Alle fünf als offen geführten Referenzen stehen vollständig im Verzeichnis, Baxter et al. 2012 mit fünf Autoren und Katalog-URL, Bleier et al. 2018 mit fünf Herausgebern, van Es et al. 2018 mit DOI, Grallert et al. in DHQ 20.1 mit URL. Die Behauptung, die DHQ-Zuweisung existiere noch nicht, war am 2026-07-23 schon überholt. Die Seitenzahlen sind gegenstandslos, weil die Operator-Regel keine setzt.

**C5 löst sich vollständig auf.** Der Review-Modus liegt auf Operator-Wunsch in der Vault-ACTIVE-WORK und war nie ein Repo-Punkt. Das ZBZ-Repositorium ist laut `gh repo view` privat, und die Fußnote sagt genau das, also ist der Text heute korrekt. Die Erfahrungswerte beziehen sich auf einen Zustand, den das Paper nicht mehr hat; Dauer- und Kostenangaben sind mit der Quantitäten-Bereinigung vom 2026-07-23 gefallen, und die Datenmengen der Projekttabelle hat der Record-Verifikations-Audit einzeln gegen die Rohdaten nachgezählt.

**C4 lag nicht dort, wo der Eintrag es vermutete.** In den Sektionen 4 und 5 steht kein ungefußnotetes Werkzeug, das die Grenze des Eintrags überschreitet. Die Lücke liegt in Abschnitt 2, wo LIDO, CMIF und PAGE XML unmittelbar neben TEI-XML stehen, das zwei Fußnoten trägt, und selbst keine hatten. Drei Fußnoten sind gesetzt.

**Die Vorlage Technology ist im Katalog, und dabei ist eine Konventionsregel gefallen.** Die Aufnahme verlangte eine Typkorrektur und eine Regeländerung. Der Entwurf führte den Dokumenttyp als Declarative mit dem Argument, die Baseline formuliere Regeln mit Begründung und keine Anweisungen. Der kanonische Papertext führt die Technology Baseline in 3.3 unter den Action Documents, und die Diagnosefrage der Typologie kommt zum selben Ergebnis: generiert ein Agent ein Artefakt der Familie mit Build-Schritt und npm-Abhängigkeit, ist der Output formal falsch, und nachzuziehen ist die Baseline. Die Zuordnung ist auf Action korrigiert, und die Grenze zum Action-Layer läuft über den Geltungsbereich.

Die Regeländerung war unvermeidlich. Der Katalog nahm eine Vorlage auf, sobald der Träger einer Funktion in mindestens zwei Repositorien in vergleichbarer Form wiederkehrt. Ein zentral gehaltenes Dokument wird einmal für eine ganze Familie geschrieben und von jedem Projekt referenziert, weshalb ein zweiter Träger niemals erscheint, gleich wie viele Projekte darauf zugreifen. Die Regel konnte diese eine Funktion also prinzipiell nicht zulassen. Für zentral gehaltene Funktionen ist das Aufnahmekriterium jetzt das zweite referenzierende Projekt statt das zweite Dokument. Was die Regel sucht, ist die Wiederkehr des Bedarfs, und eine Referenz zeigt sie so gut wie eine Kopie.

Verdrahtet ist die Vorlage in `data/promptotyping-documents.json`, in den drei Tabellen von `konvention.md`, in der Funktionsnamen-Liste von `INDEX.md` und in der Slug-Liste der `CLAUDE.md`. Die Ausnahmeliste `HELD_BACK` im Prüfskript ist leer, der Mechanismus bleibt. Der Vault hat kein Original dieser Vorlage, sie ist im Repo kanonisch.

**Vier Driftstellen aus der siebten Runde, und fünf weitere beim Nachziehen.** Erwartet waren die vier, die die Analyse gefunden hatte, das Theme-Default in `design.md`, die Glossar-Interaktion in A6 und in drei weiteren Dokumenten, und zwei Selbstwidersprüche in `architecture.md`. Beim Nachziehen kamen fünf hinzu, die niemand gesucht hatte. Die Spezifikation beschrieb die Site im Kopfsatz weiter als scrollbare Einzelseite, obwohl sie seit dem 2026-07-25 eine Seite zur Zeit zeigt. A1 trug einen Absatz über die ausgelieferte Paper-Zerlegung unter `_content/paper/`, die es seit AP1 nicht mehr gibt. Und drei Dateiangaben zeigten nach dem Zehn-Dateien-Schnitt auf `app.js`, wo `PAGES` in `registry.js`, die marked-Extension in `markdown.js` und `PRAXIS_ALIASES` samt `FUNCTION_SLUGS` in `pages-content.js` liegen. Die `CLAUDE.md` behauptete außerdem eine dritte Rasterspur mit den Überschriften der aktiven Seite; das Raster hat zwei Spuren, und das Paper trägt sein Inhaltsverzeichnis in der Seite.

**Die Lehre wiederholt sich, und sie hat eine neue Kante.** Der Refactor auf zehn Dateien war gegen Element-IDs und Anker geprüft und grün. Was er verschoben hat, sind Dateiangaben in Prosa, und die prüft kein DOM-Vergleich. Ein Wissensdokument, das eine Funktion an einer Datei festmacht, veraltet mit jedem Schnitt durch den Code, und `check_consistency.py` deckt diese Klasse nicht ab. Ein Prüfschritt, der jede in der Wissensbasis genannte Datei-Symbol-Zuordnung gegen den Code hält, wäre regelentscheidbar und ist nicht gebaut.

**`entscheidungen-offen.md` ist gelöscht.** Von zehn Einträgen trugen zwei etwas, das nirgends sonst stand, und drei waren falsch. Ein Dokument, das den Stand zweier Steuerdokumente nacherzählt, um daran Entscheidungen vorzubereiten, führt jede Aussage zweimal, und die zweite Fassung altert unbeobachtet. Das ist die Regel, die die Site in Teil 5 selbst aufstellt, angewendet auf ein Dokument der eigenen Wissensbasis. Der Operator hat gegen ein Ersatzdokument entschieden, weil aufgearbeitet werden soll statt geplant.

**Offen.** Die Use-Case-Arbeit läuft und wartet auf die Projektauswahl des Operators; siebzehn Karten stehen in der Galerie, dreizehn Zeilen in der Projekttabelle, sieben Langtexte, und die drei Mengen weichen in beide Richtungen ab. SZD-HTR fehlt in der Projekttabelle, obwohl es das durchgerechnete Beispiel in 3.4 ist. Drei Datenlücken in `data/case-studies.json`, HerData ohne Repositorium, die ZBZ-Pipeline ohne die im Paper genannte Demo-Adresse, das Austrian University Dashboard ohne beides. Die sieben Fall-Tiefenseiten sind deutsch und in ASCII-Transliteration geschrieben. Der Vault-Nachzug umfasst Konvention, Praxis, drei Fülltemplates, das Statusvokabular und die Vorlage Technology. Der Herkunftssatz zum Requirements Engineering braucht eine Fundstelle. Die Abbildungen 1 und 2 tragen die Lücken der sechsten Runde weiter, und das Zeichnen kann ich nicht leisten.

## 2026-07-26, neunte Runde — Die Galerie auf die Typologie des Papers, und die erste Prüfung, die etwas fand

Dieser Eintrag ist nachgetragen. Die Runde liegt in den Commits `b944fac`, `49f5802`, `636f914` und `c702c93`, und keiner von ihnen hat das Journal angefasst. Rekonstruiert ist er aus den Commit-Nachrichten und den Diffs.

**Das neunwertige Use-Case-Vokabular ist weg.** Es war für die Site erfunden, wurde zwischenzeitlich fälschlich einem Abschnitt des Papers zugeschrieben, und es kodierte eine andere Dimension als der Farbton, der ohnehin auf der Kartenkante saß. Gruppierung, Filter und Farbe bedeuten jetzt dasselbe, die fünf epistemischen Funktionen aus 4.2.

**Die oberste Bruchstelle trägt eine zweite Dimension, weil beide gebraucht werden und orthogonal sind.** Die Rolle sagt, wofür ein Fall steht, mit den Werten evidence, genealogy, teaching und further. Ein Genealogie-Prototyp, der gleichrangig neben einer Produktionspipeline steht, behauptet etwas Falsches über die Methode; also bricht die Galerie auf der Rolle und ordnet innerhalb eines Blocks nach der Funktion. Fünf Fälle sind dazugekommen, jeder Fall hat jetzt eine Tiefenseite, und die Seiten liegen in zwei Bauarten, fünf Abschnitte für Evidenz- und Weiterführungsfälle, drei für Genealogie- und Lehrfälle, weil bei einer aufgezeichneten Sitzung das Vorgehen der Gegenstand ist und die Datengrundlage nichts trägt.

**Die Korpuszahlen sind aus Table 1 heraus.** Jede Zahl in der Datenspalte bewegte sich mit der Arbeit, die sie erzeugt hat. Die Spalte nennt jetzt die Art des Materials und überlässt den Umfang den Repositorien, der Absatz darüber sagt warum. Die einzige Quantität im Fließtext ist die Befragtenzahl einer zitierten Erhebung, und die gehört jener Publikation. SZD-HTR ist in die Tabelle eingezogen, weil es das durchgerechnete Beispiel aus 3.4 und die Produktionspipeline des Records ist und trotzdem keine Zeile hatte.

**Zwei Korrekturen an der Evidenzlage.** Das Video auf der Karte der mittelalterlichen Rechtsgeschäfte ist ungelistet, und es von einer öffentlichen Seite zu verlinken hätte es publiziert; die Adresse ist herunter, und der Fall sagt jetzt, dass an ihm nichts einsehbar ist, womit er die am schlechtesten prüfbare Zeile der Tabelle ist. Der wiiw-Eintrag sagt in der Tabelle selbst, dass die publizierten Daten ein synthetischer Ersatz für ein vertrauliches Original sind, und führt die Entwicklung gegen synthetisierte Daten als Teil seines Beitrags.

**Dasselbe an den Tiefenseiten.** Dokumentzahlen, Dateizahlen, Korpusgrößen und die Zahl der Ansichten eines Interfaces bewegen sich mit dem nächsten Commit, und eine zum Schreibzeitpunkt verifizierte Zahl wird davon nicht dauerhaft. Sie sind ersetzt durch eine qualitative Aussage oder durch den Zeiger auf die Quelle, die den aktuellen Wert führt. Mit ihnen sind die Sitzungsgeschichten gegangen, das Publikum beim Bauen, der Defekt beim Hinzufügen einer Teilsammlung, die geplante Beitragsart; keine trug ein Argument. A7 führt seither beide Regeln, damit die nächste Seite sie nicht neu lernen muss.

**Die Site prüft jetzt auch ihre Galerie.** Drei Prüfgruppen sind dazugekommen. V4 hält Karten und Tiefenseiten gegeneinander, die Eindeutigkeit der IDs und die beiden geschlossenen Vokabulare. V5 prüft in beide Richtungen, ob jedes Projekt aus Table 1 eine Karte hat und jede Karte, die eine Tabellenzeile beansprucht, sie auch findet, und vergleicht zusätzlich die Interface-Typen beider Seiten, weil ein Widerspruch dort ein Widerspruch über die Methode wäre. Der Anker ist die Kopfzeile der Tabelle, damit ein neuer Abschnitt sie nicht verschieben kann. V6 löst jede Adresse auf, die eine Karte publiziert, und liegt hinter `--check-urls`, weil fünfzig Netzrundläufe vor jedem Commit das Skript unbenutzbar machen würden; der Standardlauf meldet die Auslassung, statt stillschweigend zu bestehen.

**Was die Prüfungen beim ersten Lauf fanden.** V5 meldete drei Projekte der Tabelle ohne Karte, VetMedAI Wissensbilanz, wiiw Patent Analysis und die mittelalterlichen Rechtsgeschäfte; die zehn Projekte mit Karte stimmten ausnahmslos mit der Tabelle über ihre Interface-Typen überein. V6 meldete zwei tote Adressen. Die Kulturpool-Karte zeigte auf ein Repositorium, das es nicht gibt, das Projekt liegt unter `chpollin/kulturpool-demo` und hat eine antwortende Pages-Site, weshalb die Karte zusätzlich eine funktionierende Demo-Adresse bekam. Die HerData-Karte bewarb eine Demo, die 404 antwortet, und das Repositorium dahinter ist privat ohne Pages-Site; die Adresse ist entfernt, weil ein toter Link mehr behauptet als kein Link.

**Ein Befund über diese Runde selbst, sichtbar erst beim Nachtragen.** Der Umbau des Vokabulars hat `knowledge/verification.md` nicht mitgenommen. Die Beschreibung von V4 nennt dort weiterhin `useCase` und `_meta.use_case_labels`, während das Skript `role` gegen `role_labels` prüft und `use_case_labels` in den Daten nicht mehr existiert. Die Verifikationsdokumentation beschreibt damit eine Prüfung, die es so nicht gibt, und das ausgerechnet in dem Dokument, das die Prüfbarkeit der Site verwaltet.

## 2026-07-26, zehnte Runde — Die offene Liste abgearbeitet, in zwei Wellen

Ziel war, jeden offenen Punkt zu schliessen, für den das Repo die Mittel hat. Der Zuschnitt ist beim Planen zweimal gewandert, und beide Male hat die Vorerkundung ihn korrigiert.

**Disjunkter Dateibesitz ist keine Unabhängigkeit.** Mein erster Plan führte vier gleichzeitige Spuren mit sauber getrennten Dateilisten und nannte das parallel. Das war falsch. Die Prüfgruppe für die Ankerauflösung liest genau die Tabellen, die der Routing-Umbau gerade verschiebt, und der Glossar-Generator hängt an der Datei, die die Begriffsarbeit ändert. Getrennter Besitz verhindert den Schreibkonflikt und sagt nichts über die Reihenfolge. Die Runde läuft deshalb in zwei Wellen, und die Kopplungen stehen im Plan, statt sich zur Laufzeit zu zeigen.

**Vier Punkte des Plans waren nach der Vorerkundung anders zu schneiden.** `_content/glossar.md` ist byteweise vollständig aus `data/glossar.json` ableitbar, bis auf einen dreizehnzeiligen Vorspann, was den Generator zu einer kleinen Arbeit macht. Die acht Prüfkriterien für `user-stories.md` existierten nirgends im Repo, die Verweiskette lief im Kreis. Die Subpath-Präfixtabelle stand dreimal im Code, und die dritte Kopie war bereits sieben Einträge hinter der ersten. Und die fünf angeblich toten Glossarbegriffe waren nicht tot, weil das Register die volle Nominalphrase als Wortfolge sucht und lange Begriffsnamen deshalb prinzipiell nie treffen.

**A7 war präziser als angenommen.** Die sieben deutschen Tiefenseiten sind sämtlich Evidenzfälle und tragen deshalb fünf Abschnitte samt dem verpflichtenden Grenzen-Abschnitt. Der Abschnitt `## Links` fiel nach der Regel, dass Adressen allein auf der Karte stehen, und dabei kam heraus, dass die HerData-Seite die als tot erkannte Demo-Adresse weiter bewarb, nachdem die Karte sie längst verloren hatte. Eine Regel, die nur für einen von zwei Trägern derselben Aussage formuliert ist, lässt den anderen stehen. Bei `source` und `mirrored` ist entschieden, dass eine inhaltlich neu geschriebene Seite kein Spiegel mehr ist; Präzedenz sind die beiden Skill-Dateien vom 2026-07-20.

**`_content/MANIFEST.md` wollte nicht repariert werden.** Die Datei erzählte den Stand von `_content/` und `data/` nach, also dieselbe Bauart wie das im Juli gelöschte `entscheidungen-offen.md`. Drei der gestrichenen Blöcke waren nicht nur redundant, sondern falsch: eine Ausschlussliste mit neun Fällen, von denen acht inzwischen in der Galerie stehen, eine Vault-Quelle für M3GIM, die es nicht gibt, und ein Lücken-Abschnitt über ein Feld, das die Galerie nicht mehr führt. Geblieben ist, was nur dort steht, die Spiegelungsregeln, die Richtung der Source of Truth pro Datei und die Stellen, an denen das Repo dem Vault voraus ist. Der eine noch gültige Ausschlussgrund ist in die Kuratierungsnotiz der Galeriedaten gewandert, also in die Quelle, die über die Aufnahme entscheidet.

**Die Adresse, die das Paper zusagt, stand nicht auf der Karte.** Fussnote 7 nennt das Verifikations-Frontend der ZBZ-Pipeline als öffentlich, und die Karte trug überhaupt keine Adresse. Zusätzlich verlinkte die Karte des Austrian University Dashboard Teil 1 der Videoreihe, während der Bau in Teil 2 stattfindet, was Fussnote, README, `project.md` und das zweite Skriptum übereinstimmend sagen. Beide Adressen antworten, es war also kein toter Link, sondern ein Zeiger auf das falsche Ziel, und diese Klasse findet keine URL-Prüfung.

**Die Auflösung der Subpaths steht jetzt einmal in der Anwendung.** `ANCHOR_OWNER` ist zu einer Familientabelle geworden, die Präfix, besitzende Seite und Subpath-Segment zusammen führt, und sowohl der Router als auch `resolveTemplateUrl` lesen sie. Die nackten Slugs kommen aus `PAGES`. Hartkodiert bleibt, was wirklich besonders ist, `konvention` auf `konvention-v0.1`, der Snapshot-Suffix auf einem Vorlagenpfad, und `paper/{x}` auf `abschnitt-{x}`. In `404.html` steht die Abbildung weiterhin ein zweites Mal, weil der Umbau zum Durchreicher eine offene Operator-Entscheidung ist.

**Geprüft wurde diesmal auch im Browser.** Der Vergleich der Auflösungslogik lief in Node gegen den Stand aus `HEAD`, über 200 publizierte Adressformen und 187 Anker, ohne Regression. Danach ein Server, der sich wie GitHub Pages verhält und für jeden Nichtdatei-Pfad `404.html` mit Status 404 ausliefert, und zehn Adressformen im Headless-Chromium; jede landet auf der richtigen Seite, ein Unsinnspfad auf der Nicht-gefunden-Seite. Die Lehre der fünften Runde war, dass gerendertes DOM Struktur prüft und nicht Übereinstimmung. Die Ergänzung dazu lautet, dass eine Auflösungslogik, die im Node-Sandkasten stimmt, noch nichts über die Kette aus Server, Weiterleitung und Shell sagt.

**QUS widerlegt, was die Site über QUS sagte.** Das Framework führt dreizehn Kriterien in drei Kategorien, sein Werkzeug AQUSA implementiert fünf Analyzer, und die Auswahlregel schliesst die semantischen Kriterien ausdrücklich aus, weil sie Inhaltsverständnis verlangen. Acht Kriterien in der Aufteilung vier plus drei trifft weder Zahl noch Einteilung. Schwerer wiegt, dass die Quelle die eigentliche Aussage des Eintrags nicht trägt: alle dreizehn beurteilen die intrinsische Qualität des Story-Textes, keines fragt nach der Bestätigung durch den benannten Nutzer. Der epistemische Status bleibt eine Setzung der Site und wird als solche ausgewiesen.

**Drei der fünf Prägungen haben einen etablierten Namen.** Abgestufte Rechte sind Least Privilege. Die Rollensimulation innerhalb einer Sitzung heisst role play, und deren eigene Literatur begrenzt den Anspruch, weil eine per Prompt gesetzte Rolle eine Verteilung bleibt, die das Gespräch weiter umformt, statt einer erzwungenen Grenze. Statisches Artefakt neben eigenständigem Datensatz ist ein Research Compendium, was die Anordnung genauer trifft als FAIR, weil FAIR Eigenschaften von Daten normiert und nicht die Form eines Repositoriums. Zwei Negativbefunde stehen als solche im Vault, von Toulmin 1958 und Winograd 1975 war keine offene zitierfähige Fassung zu bekommen.
