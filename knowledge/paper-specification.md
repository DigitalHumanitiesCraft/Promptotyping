...
5. **Closed on 2026-08-19.** The two owed footnotes of §2.3.1 are written and sourced; see decision 29. The entry keeps its number so that the references to the flag set in the journal and the work-block list stay valid.
6. **The project URLs are entered; the ZBZ repository pointer remains an editorial decision.** Footnotes 25 (M³GIM) and 26 (Notker) received repository and interface URLs on 2026-08-09, footnote 24 (Jeanne Hersch) the public corpus interface `https://chpollin.github.io/zbz-ocr-tei/` on 2026-08-19; no `[Insert project URL.]` marker survives. What stays open is where the ZBZ repository pointer that the five-chapter text carried belongs, and whether it can be given at all, since `chpollin/zbz-ocr-tei` is private while its GitHub Pages output is public. Table 1 in §3.2 carries no URL column, so the placements actually available are footnote 24 beside the interface URL and the rights and repository-visibility list of Section 22; omitting the pointer while the repository stays private is the third option.
...
8. **The announcement and the Author's Note still name the GPT model differently (state 2026-08-19).** The manuscript declaration was reconciled with the repository's provenance records on 2026-08-19 (decision 30) and now names five language models beside the image generator, the harness, and the two writing tools. The public announcement of 2026-07-31 names Claude Opus 5, Claude Fable 5, and GPT-5.6 Sol, so it is now the shorter of the two lists by a wider margin. One difference is beyond decision from the record, namely whether the GPT model's full name is GPT-5.6 Sol, which no repository provenance record carries. The AI-use declaration is the one place where an imprecision is itself a research-integrity problem, so only the operator can settle the name and say whether the announcement is to be corrected. Until then the manuscript's list governs, because it is the declaration of record.
