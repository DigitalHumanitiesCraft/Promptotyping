...
|CorrespExplorer|CMIF/TEI, 11,576 letters|Exploration|First complete vault (7 docs, 46 journal phases, 74+ tests)|[DigitalHumanitiesCraft/CorrespExplorer](https://github.com/DigitalHumanitiesCraft/CorrespExplorer)|
...
|HerData|TEI/Wikidata, 1,793 letters|Exploration|Gender-focused network with "Map Bias" transparency section|[chpollin/HerData](https://github.com/chpollin/HerData)|
...
|VetMedAI Wissensbilanz|76 Excel files, 22 universities|Exploration|Largest knowledge base (41 documents, hierarchical structure)|[DigitalHumanitiesCraft/vetmed-wissensbilanz](https://github.com/DigitalHumanitiesCraft/vetmed-wisbensbilanz)|
|Kulturpool Explorer|Kulturpool API, ~19k objects|Exploration|Parallel agent orchestration (2 Claude Code instances)[^vid-kulturpool]|[chpollin/vkm-explorer](https://github.com/chpollin/vkm-explorer)|
...
|Medieval Legal Transactions|~6,400 TEI-XML|Edition|Agentic Edition Workflow (381 regression tests)[^vid-sugw]|[chpollin/db_for_medieval_legal_transactions](https://github.com/chpollin/db_for_medieval_legal_transactions)|
|M³GIM|RiC-O/JSON-LD, archival data|Exploration, Capture|Data model evolves through interface (49 decisions)|[DigitalHumanitiesCraft/m3gim](https://github.com/DigitalHumanitiesCraft/m3gim)|
|ZBZ OCR/TEI Pipeline|286 PDFs, ~4,150 pages|Verification, Edition|Full epistemic infrastructure (16 knowledge docs)|[chpollin/zbz-ocr-tei](https://github.com/chpollin/zbz-ocr-tei)|
...
|Klawiter Bibliography Rescue|MediaWiki SQL/BLOB, 6,296 entries|Verification, Capture|Data rescue: 7-stage pipeline + EIL curation interface (1 day, 264 tests)[^vid-klawiter]|[chpollin/klawiter-rescue](https://github.com/chpollin/klawiter-rescue)|
...
|Austrian University Dashboard|76 Excel files|Exploration|Didactic baseline (full process in video)[^vid-intro]|—|
...
**Insight 9: The agent as repository interface.** In the **Medieval Legal Transactions edition**, Claude Code reads 8 knowledge documents, then executes a defined workflow: place TEI-XML → validate → fix → commit → update registers → transform → run 381 regression tests → build frontend → verify.[^vid-sugw] The editor interacts with ~6,400 documents through the agent. Interpretive tasks (person identification across documents) are explicitly excluded.

**Insight 10: The interface reveals the data model's limits.** **M³GIM** (25+ sessions, 49 documented decisions) builds an 8-tab archive explorer for RiC-O/JSON-LD archival data. The interface reveals what the current data model can and cannot express — making it an instrument for iterative model refinement rather than a presentation of a finished model.

**Insight 11: Full epistemic infrastructure.** The **ZBZ OCR/TEI Pipeline** processes 286 PDF scans (~4,150 pages) through nine stages with 16 knowledge documents plus 3 Action Documents. Over 30 Python scripts were generated and maintained entirely through Claude Code. Verification is handled through Promptotyping Interfaces at each pipeline stage. OCR processing cost less than 10 EUR. The project was built in approximately six weeks (128 commits). The **VetMedAI Wissensbilanz** (41 documents in a hierarchical structure) confirms the pattern with different data and introduces Learnings as a distinct Process Document type.
