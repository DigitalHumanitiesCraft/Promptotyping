---
type: assertion
topics:
- '[[Frame]]'
status: grounded
checked:
  validation: 2026-07-23
grounding:
- '[[20_distillates/publications/wilkinson-2016-fair-principles#^s1]]'
- '[[20_distillates/publications/wilkinson-2016-fair-principles#^s2]]'
created: 2026-07-23
updated: 2026-07-23
---

# The FAIR principles are a concise, measurable set for making data Findable, Accessible, Interoperable, and Reusable.

## Statement

Wilkinson et al. state the FAIR Data Principles as a concise, measurable set of guidelines for enhancing the reusability of data holdings, under the four headings Findable, Accessible, Interoperable, and Reusable.

## Support

- [[20_distillates/publications/wilkinson-2016-fair-principles#^s1]] — a concise, measurable set of principles to enhance reusability.
- [[20_distillates/publications/wilkinson-2016-fair-principles#^s2]] — Findable, Accessible, Interoperable, and Reusable.

## Related

- [[30_assertions/fair-emphasises-machine-actionability-of-data]]
